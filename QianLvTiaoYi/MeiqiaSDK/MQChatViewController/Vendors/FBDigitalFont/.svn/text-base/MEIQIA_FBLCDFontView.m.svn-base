...
@interface MEIQIA_FBLCDFontView ()
@property (nonatomic, copy) NSArray *symbols;
@end

@implementation MEIQIA_FBLCDFontView
...
- (void)drawRect:(CGRect)rect
{
    ...
    if (self.drawOffLine) {
        ...
        for (i = 0; i < [self.symbols count]; i++) {
            [MEIQIA_FBLCDFont drawSymbol:FBFontSymbol8
    ...
    for (i = 0; i < [self.symbols count]; i++) {
        [MEIQIA_FBLCDFont drawSymbol:[[self.symbols objectAtIndex:i] intValue]
                   edgeLength:self.edgeLength
                    lineWidth:self.lineWidth
                   startPoint:CGPointMake(x + i * l, y)
                    inContext:ctx];
