...
@implementation MEIQIA_FBBitmapFontView
...
{
    _text = text;
    self.symbols = [MEIQIA_FBFontSymbol symbolsForString:text];
...
- (NSInteger)numberOfHorizontalDot
{
    ...
    for (NSNumber *number in self.symbols) {
        ...
        totalDotsFromSymbols += [MEIQIA_FBBitmapFont numberOfDotsWideForSymbol:symbol];
    }
...
//    CGFloat l = (self.edgeLength + self.margin) * (5 + self.numberOfPaddingDotsBetweenDigits);
    ...
    for (i = 0; i < [self.symbols count]; i++) {
        ...
        [MEIQIA_FBBitmapFont drawSymbol:[[self.symbols objectAtIndex:i] intValue]
        ...
        CGFloat numberWide = [MEIQIA_FBBitmapFont numberOfDotsWideForSymbol:[[self.symbols objectAtIndex:i] intValue]];
    ...
    [MEIQIA_FBBitmapFont drawBackgroundWithDotType:self.dotType 
                                      color:self.offColor 
                                 edgeLength:self.edgeLength 
                                     margin:self.margin 
